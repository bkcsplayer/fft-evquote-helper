...
export default function DiagnosticFlow() {
  ...
  function validateStep1() {
    ...
    if (!problemDesc.trim()) return t('svc.diagnostic.err.problem_desc')
  ...
  async function onSubmit() {
    ...
    try {
      const res = await api.post('/public/services/bookings', {
        ...
        inverter_info: inverterInfo.trim() || null,
  ...
  return (
    <QuoteShell>
      ...
      {step === 0 ? (
        <div className="rounded-3xl border border-zinc-100 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-bold tracking-tight text-zinc-900">{t('svc.diagnostic.intro_title')}</h2>
          <p className="mt-2 text-sm leading-relaxed text-slate-600">{t('svc.diagnostic.intro_body')}</p>
          ...
          <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-700">{t('svc.diagnostic.intro_price_body', { price: 179 })}</p>
          </div>

          <div className="mt-4">
            <div className="text-sm font-semibold text-slate-900">{t('svc.diagnostic.intro_how_title')}</div>
            <ol className="mt-2 space-y-1.5">
              {[t('svc.diagnostic.intro_how_1'), t('svc.diagnostic.intro_how_2'), t('svc.diagnostic.intro_how_3')].map((s, i) => (
          ...
          <div className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900">{t('svc.diagnostic.intro_home')}</div>
      ...
      {step === 1 ? (
        <div className="rounded-3xl border border-zinc-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            ...
            <label className="block">
              <div className="text-sm font-medium text-slate-800">{t('svc.diagnostic.inverter_info')}</div>
              <input value={inverterInfo} onChange={(e) => setInverterInfo(e.target.value)} className="mt-1 w-full rounded-2xl border border-zinc-200 px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-600" placeholder={t('svc.diagnostic.inverter_info_ph')} />
            </label>

            <div className="block">
              <div className="text-sm font-medium text-slate-800">{t('svc.diagnostic.problem_label')}</div>
                ...
                {CHIP_KEYS.map((k) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => toggleTag(k)}
                      className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors ${active ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}
                    >
                      {t(`svc.diagnostic.chip.${k}`)}
                    </button>
                  )
                })}
              </div>
              <textarea
                ...
                className="mt-2 w-full rounded-2xl border border-zinc-200 px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-600"
                rows={3}
                placeholder={t('svc.diagnostic.problem_desc_ph')}
              />
            </div>
            ...
            <div className="block">
              <div className="text-sm font-medium text-slate-800">{t('svc.common.choose_time')}</div>
      ...
      {step === 2 ? (
        <div className="rounded-3xl border border-zinc-100 bg-white p-6 shadow-sm">
          ...
          <DisclaimerBlock text={t('svc.diagnostic.disclaimer')} accepted={accepted} onChange={setAccepted} />
