...
export default function CaseDetail() {
  ...
  useEffect(() => {
    if (loading) return
    const hash = (loc.hash || '').toLowerCase(); if (!hash) return
    const targetId = hash.replace('#', '').trim(); if (!targetId) return
  ...
  async function completeSurvey() {
    ...
    try { await api.patch(`/cases/${id}/survey/complete`, { survey_notes: surveyNotes.trim() || null }); setSurveyNotes(''); await load(); setTimeout(() => document.getElementById('quote')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50) }
  ...
  async function uploadSurveyPhoto() {
    if (!surveyPhotoFile) return; setBusy(true); setError('')
    try { const form = new FormData(); form.append('file', surveyPhotoFile); const qp = new URLSearchParams(); qp.set('category', surveyPhotoCategory); if (surveyPhotoCaption.trim()) qp.set('caption', surveyPhotoCaption.trim()); await api.post(`/cases/${id}/survey/photos?${qp.toString()}`, form); setSurveyPhotoFile(null); setSurveyPhotoCaption(''); await load() }
  ...
  async function uploadInstallationPhoto() { if (!installPhotoFile) return; setBusy(true); setError(''); try { const form = new FormData(); form.append('file', installPhotoFile); if (installPhotoCaption) form.append('caption', installPhotoCaption); await api.post(`/cases/${id}/installation/photos`, form); setInstallPhotoCaption(''); setInstallPhotoFile(null); await load() } catch (e) { setError(e?.response?.data?.detail || 'Failed to upload installation photo') } finally { setBusy(false) } }
      ...
      <div className="animate-fade-in">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-xl font-bold tracking-tight text-slate-900">{data?.reference_number || 'Case'}</h1>
          </div>
        </div>
          ...
          <div className="mt-5 space-y-5">
            ...
            <SectionCard tone="slate" title="Customer" subtitle="People & case basics">
              <div className="grid gap-2 text-sm">
                ...
                <div><span className="text-slate-500">Status:</span> <Pill tone={toneForCaseStatus(data.status)}>{data.status}</Pill></div>
                {tokenLink && (
                  <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Customer link (dev)</div><div className="mt-1 break-all text-xs text-slate-700">{tokenLink}</div></div>
                )}
              </div>
            </SectionCard>
              ...
              <LockedSection locked={!canUploadSurveyPhotos(data.status)} reason="Survey photos unlock after the survey is completed.">
                <div className="mt-5 rounded-xl border bg-slate-50 p-4">
                  ...
                  <div className="mt-3 grid gap-2">
                    ...
                    <input type="file" onChange={(e) => setSurveyPhotoFile(e.target.files?.[0] || null)} className="text-sm" />
                    <button type="button" disabled={busy || !surveyPhotoFile} onClick={uploadSurveyPhoto} className={btnPrimary}>Upload photo</button>
                  </div>
                  <div className="mt-3 space-y-2">
                    {surveyPhotos.map((p) => (
                      <div key={p.id} className="rounded-xl border bg-white p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="text-xs text-slate-600"><span className="font-semibold">{p.category}</span> • {new Date(p.created_at).toLocaleString()}</div>
                          <button type="button" disabled={busy} onClick={() => deleteSurveyPhoto(p.id)} className="rounded-lg border bg-white px-2.5 py-1.5 text-xs font-semibold text-rose-600 transition-all hover:bg-rose-50 disabled:opacity-60">Delete</button>
                        </div>
                        {p.caption && <div className="mt-1 text-xs text-slate-500">{p.caption}</div>}
                        <div className="mt-2 overflow-hidden rounded-lg border bg-slate-50">
                          <button type="button" onClick={() => setPreview({ src: `/${p.file_path}`, title: p.file_name || p.category || 'Survey photo', subtitle: p.caption || p.category })} className="block w-full" title="Click to preview">
                            <img src={`/${p.file_path}`} alt={p.file_name} className="h-40 w-full object-cover" />
                          </button>
                        </div>
                      </div>
                    ))}
                    {surveyPhotos.length === 0 && <div className="py-4 text-center text-xs text-slate-400">No photos.</div>}
                  </div>
                </div>
              </LockedSection>
            </SectionCard>
                    ...
                    <div className="grid gap-2 rounded-xl border bg-slate-50 p-4 text-sm">
                      ...
                      {data.active_quote.signature && (
                        <div className="rounded-xl border bg-white p-3">
                          <div className="text-xs text-slate-500">Signed by</div><div className="mt-0.5 font-bold text-slate-900">{data.active_quote.signature.signed_name}</div>
                          <div className="mt-1 text-xs text-slate-500">{new Date(data.active_quote.signature.signed_at).toLocaleString()}</div>
                          {String(data.active_quote.signature.signature_data || '').startsWith('data:image') && <img alt="Signature" src={data.active_quote.signature.signature_data} className="mt-2 max-h-40 w-full rounded-xl border bg-white object-contain" />}
                        </div>
                      )}
              ...
              <LockedSection locked={!canEditInstallation(data.status)} reason="Installation unlocks after permit is approved.">
                <SectionCard tone="emerald" title="Installation" subtitle="Step 4 — Schedule & complete install">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    ...
                    <label className="block md:col-span-2"><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Notes</span><input value={installNotes} onChange={(e) => setInstallNotes(e.target.value)} disabled={busy || (isAtOrAfter(data.status, 'installed') && !installationDateIssue)} className={inputCls} /></label>
                    {hasPendingInstallRequest && <div className="md:col-span-3 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs font-semibold text-amber-800">Pending reschedule request — confirm or reject first.</div>}
                    {installationScheduledInFuture && <div className="md:col-span-3 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs font-semibold text-amber-800">Too early to mark installed — scheduled time is in the future.</div>}
                  ...
                  <div className="mt-5 rounded-xl border bg-slate-50 p-4">
                    ...
                    <div className="mt-3 grid gap-2">
                      <input value={installPhotoCaption} onChange={(e) => setInstallPhotoCaption(e.target.value)} className={inputCls + ' mt-0'} placeholder="caption (optional)" />
                      <input type="file" onChange={(e) => setInstallPhotoFile(e.target.files?.[0] || null)} className="text-sm" />
                      <button type="button" disabled={busy || !installPhotoFile} onClick={uploadInstallationPhoto} className={btnPrimary}>Upload photo</button>
                    </div>
                    <div className="mt-3 space-y-2">
                      {installPhotos.map((p) => (
                        <div key={p.id} className="rounded-xl border bg-white p-3">
                          <div className="flex items-start justify-between gap-2">
                            <div className="text-xs text-slate-600"><span className="font-semibold">{p.file_name}</span> • {new Date(p.created_at).toLocaleString()}</div>
                            <button type="button" disabled={busy} onClick={() => deleteInstallationPhoto(p.id)} className="rounded-lg border bg-white px-2.5 py-1.5 text-xs font-semibold text-rose-600 transition-all hover:bg-rose-50 disabled:opacity-60">Delete</button>
                          </div>
                          {p.caption && <div className="mt-1 text-xs text-slate-500">{p.caption}</div>}
                          <div className="mt-2 overflow-hidden rounded-lg border bg-slate-50">
                            <button type="button" onClick={() => setPreview({ src: `/${p.file_path}`, title: p.file_name || 'Installation photo', subtitle: p.caption || 'Installation photo' })} className="block w-full" title="Click to preview">
                              <img src={`/${p.file_path}`} alt={p.file_name} className="h-40 w-full object-cover" />
                            </button>
                          </div>
                        </div>
                      ))}
                      {installPhotos.length === 0 && <div className="py-4 text-center text-xs text-slate-400">No photos.</div>}
                    </div>
                  </div>
                </SectionCard>
              </LockedSection>
            ...
            <SectionCard tone="rose" title="Admin override (password required)" subtitle="Emergency fixes — logs history and notifies customer via SMS">
              <div className="grid gap-3 md:grid-cols-6">
                ...
                <label className="block md:col-span-2"><span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Set case status to</span><select value={overrideToStatus} onChange={(e) => setOverrideToStatus(e.target.value)} className={selectCls + ' focus:border-rose-500 focus:ring-rose-500/20'}>
                  <option value="pending">pending</option><option value="survey_scheduled">survey_scheduled</option><option value="survey_completed">survey_completed</option><option value="quoting">quoting</option><option value="quoted">quoted</option><option value="customer_approved">customer_approved</option><option value="permit_applied">permit_applied</option><option value="permit_approved">permit_approved</option><option value="installation_scheduled">installation_scheduled</option><option value="installed">installed</option><option value="completed">completed</option><option value="cancelled">cancelled</option>
                </select></label>
  ...
  return (
    <div className="fixed inset-0 z-50 animate-fade-in">
      ...
      <div className="relative mx-auto flex h-full max-w-5xl items-center justify-center p-4">
        <div className="w-full overflow-hidden rounded-2xl bg-white shadow-2xl animate-slide-up">
